...
import CarOptions from './DistanceMatrix'
          ...
          renderItem={({item:{id,title,multiplier,image},item}) => (
            <TouchableOpacity style={tw`flex-row justify-between items-center px-8 ${id===selected?.id && 'bg-gray-200'}`}
             onPress={() => setSelected(item)}>
              ...
              <Text className='text-xl '>{new Intl.NumberFormat ('vi-VN', {style: 'currency', currency: 'VND', maximumFractionDigits: 9}).format (distance*5000*multiplier)}</Text>
            </TouchableOpacity>
